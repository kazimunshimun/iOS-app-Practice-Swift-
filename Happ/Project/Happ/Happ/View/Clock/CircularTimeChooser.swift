//
//  CircularTimeChooser.swift
//  Happ
//
//  Created by Anik on 14/9/19.
...
@objc public  protocol ClockDelegate {
    ...
    @objc optional func timesUpdated(_ clock:CircularTimeChooser, startDate:Date,  endDate:Date  ) -> ()
    //Executed after the user lifts their finger from the control.
    @objc optional func timesChanged(_ clock:CircularTimeChooser, startDate:Date,  endDate:Date  ) -> ()
}
...
@IBDesignable
open class CircularTimeChooser : UIControl{
    
    open var delegate:ClockDelegate?
    ...
    let repLayer:CAReplicatorLayer = {
        ...
        r.instanceTransform =
            CATransform3DMakeRotation(
                CGFloat(2*Float.pi) / CGFloat(r.instanceCount),
                0,0,1)
    ...
    let twoPi =  CGFloat(2 * Float.pi)
    let fourPi =  CGFloat(4 * Float.pi)
    var headAngle: CGFloat = 0{
        didSet{
            if (headAngle > fourPi  +  CGFloat(Float.pi)){
                headAngle -= fourPi
            }
            if (headAngle <  CGFloat(Float.pi) ){
                headAngle += fourPi
            }
        }
    }
    
    var tailAngle: CGFloat = 0.7 * CGFloat(Float.pi) {
        didSet{
    ...
    open var headBackgroundColor = UIColor.white.withAlphaComponent(1.0)
    open var tailBackgroundColor = UIColor.white.withAlphaComponent(1.0)
    ...
    func disabledFormattedColor(_ color:UIColor) -> UIColor{
        return disabled ? UIColor.lightGray : color
    }
    ...
    var buttonRadius:CGFloat { return 44/2 /*pathWidth / 2*/ }
    ...
    var strokeColor: UIColor {
        ...
        set(strokeColor) {
            trackLayer.strokeColor = strokeColor.withAlphaComponent(1.0).cgColor
    ...
    func timeToAngle(_ date: Date) -> Angle{
        ...
        return medStepFunction(CGFloat(M_PI_2 - ( min / (12 * 60)) * 2 * M_PI), stepSize: CGFloat( 2 * Float.pi / (12 * 60 / 5)))
    }
    
    // input an angle, output: 0 to 4pi
    func angleToTime(_ angle: Angle) -> Date{
        let dAngle = Double(angle)
        let mAngle = CGFloat(Double.pi/2 - dAngle)
        let minAngle = CGFloat(mAngle / CGFloat(2 * Float.pi))
        let hour = CGFloat(12 * 60)
        let min = CGFloat(minAngle * hour)
    ...
    open func update() {
        ...
        strokeColor = ColorUtils.colorRGB(215.0, 223.0, 234.0)
    ...
    func imageLayer(_ imageName: String, _ width: CGFloat, _ height: CGFloat) -> CALayer {
    ...
    let fromShapeLayer = CAShapeLayer()
    ...
    func updateHeadTailLayers() {
        ...
        /*
        if let image = UIImage(named: "light_to") {
            fromShapeLayer.contents = image.cgImage
            fromShapeLayer.position = topHeadLayer.center
            topHeadLayer.addSublayer(fromShapeLayer)
        }
 */
        let startImage = imageLayer("light_from", 15, 21)
        startImage.position = topTailLayer.center
        topTailLayer.addSublayer(startImage)
        
        let endImage = imageLayer("light_to", 20, 22)
        endImage.position = topHeadLayer.center
        topHeadLayer.addSublayer(endImage)
        /*
 ...
 */
    ...
    func updateWatchFaceNumerals() {
        ...
        for i in (1 ... 12){
            if i % 3 == 0 {
                ...
                let rotation = CGFloat(Float(i) * step)
                l.position = CGVector(from:origin, to:startPos).rotate(rotation).add(origin.vector).point.checked
    ...
    func updateWatchFaceTitle(){
        ...
        let hour = String(format: "%02d", fiveMinIncrements / 12)
        let minute = String(format: "%02d", (fiveMinIncrements % 12) * 5)
        titleTextLayer.string = "\(hour):\(minute)"
